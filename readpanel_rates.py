...
def readratescmd(lookback, samples):
...
def main():
    ...
    try:
      ...
      ## lets not read the thresholds for now, but implement it later
...
#      env_data = {'time':timestamp,'calenv':calenvdata, 'hvenv':hvenvdata}
      ...
      readcaldigi = 0
      if (lastrun+1)%2==0: 
          readcaldigi = 1 ## alternate between reading the HV and Cal digis

      lookback = 1000
      samples = 100
      cmd = readratescmd(lookback, samples)
      sercal.write(cmd)

      filename="test"
      fout = open(os.path.join(rundir,filename),"w")
      
      serialread_rate(sercal)
